...
const MTC_FRAMEWORKS = [
  {
    ...
    whyWrong: "Clever people are better at building a case, so they defend a wrong first impression more convincingly than a beginner could.",
    avoid: "Treating 'critical' as 'cynical'. Doubting everything by habit is not the same as checking something properly.",
    danger: "Overused, it turns into endless weighing up, or doubting things out loud to look sharp, instead of reaching a decision.",
    example: "A friend forwards a health claim with a link. Before passing it on you check who published it and what they compared it against.",
  ...
  },
  {
    ...
    problem: "Turns a problem too big to hold in your head into parts small enough to look at one at a time.",
    whyWrong: "People split a problem where the joins are easiest to see, not where the real causes sit.",
    avoid: "Splitting it so finely that you stop seeing how the parts affect each other.",
    danger: "Risky when the parts affect each other. Studying them separately can miss what only happens when they run together.",
    example: "A washing machine leaks. Rather than replace it, you run it empty to see whether the water comes from the door seal, the hose or the pump.",
    expertUse: "Splits the problem along the line that separates the thing being tested, not along whichever line is easiest to describe.",
  },
  {
    ...
    example: "A meeting is stuck on where to cut costs. Someone asks what you would do if the budget doubled, and the answers show what actually matters.",
  ...
  },
  {
    ...
    example: "A shop pays staff a bonus for every complaint they resolve. Complaints start being logged that used to be sorted quietly on the spot.",
  ...
  },
  {
    ...
    danger: "Over-planning for a single predicted future leaves you fragile if that future doesn't materialise.",
    example: "A new café sells coffee at close to cost for a year, to become the place people already walk past on the way to work.",
  ...
  },
  {
    ...
    example: "You cannot know whether it will rain on Saturday. You plan for a one-in-three chance by taking a coat, rather than deciding it will or will not.",
  ...
  },
  {
    ...
    core: "Stating an idea in a way that could be shown wrong, then looking for the evidence that would show it.",
    problem: "Distinguishes a real effect from noise, and a real cause from a coincidence.",
    whyWrong: "People design tests that can only agree with them, and never a test that could prove them wrong.",
  ...
  },
  {
    ...
    core: "Understand the person, name the real problem, think up options, build a rough version, test it — iterating with real users instead of assumptions about them.",
  ...
  },
  {
    ...
    example: "Everyone writes ideas down silently for five minutes before anyone speaks, so the first confident voice does not set the direction for the room.",
  ...
  },
  {
    ...
    example: "A hiring panel scores every application against the same four requirements, then interviews the top three, instead of picking whoever felt right.",
  ...
  },
  {
    ...
    core: "Breaking a problem into steps clear enough that someone else, or a machine, could follow them.",
    ...
    example: "A checklist turns 'get everyone out of the house on time' into named steps, so nothing depends on remembering it while rushing.",
    expertUse: "Abstracts the specific case into a general pattern, then designs a procedure that generalises.",
  },
  {
    ...
    example: "Your car will not start. The likeliest cause is the battery, so you test that first rather than starting with the rarer faults.",
  ...
  },
  {
    ...
    example: "The rule says anything over £500 needs a second signature. This order is £600. So it needs a second signature, whatever anyone thinks of it.",
  ...
  },
  {
    ...
    core: "Generalising a pattern from repeated observations.",
    problem: "Lets you form useful general rules from finite experience.",
    whyWrong: "People generalise from a small or biased sample and treat the generalisation as a law.",
    avoid: "Ignoring how the sample was selected — a biased sample produces a biased generalisation no matter how large.",
    danger: "The generalisation can break exactly when it matters most, in the tail case you never observed.",
    example: "Every bill from this supplier has arrived on the first of the month. You expect the next one then, knowing one late bill would break the pattern.",
    expertUse: "Treats the pattern as true for now, and goes looking for the case that would break it.",
  },
  {
  ...
  },
  {
    ...
    example: "You pay for travel insurance you will probably never claim on, because the rare case it covers would cost more than many years of premiums.",
  ...
  },
  {
    ...
    core: "Analysing strategy where the outcome depends on what other rational actors choose too.",
    ...
    example: "Two neighbouring shops keep cutting prices to win customers. Both end up earning less, and neither can raise prices first without losing trade.",
  ...
  },
  {
    ...
    avoid: "Assuming each new case fits the pattern, without checking the details that would show it does not.",
  ...
  },
  {
    ...
    example: "You looked for your keys and did not find them. That is weak evidence they are not there, if you only checked the obvious places.",
  ...
  },
  {
    ...
    core: "Recognising the regular ways your own mind bends a judgement.",
    ...
    example: "Someone keeps paying for a gym they never visit because of what they have already spent. Being able to name the mistake is not the same as stopping.",
  ...
  },
  {
    ...
    problem: "Prevents optimising a single metric into a decision that everyone recognises afterward as wrong.",
    ...
    example: "Two flatmates split bills. One wants a rule that stays fair every month; the other wants whatever leaves both better off right now. Both are reasonable.",
  ...
  },
  {
    ...
    example: "A parent notices nobody near the school offers care after clubs finish, and that every other parent has quietly arranged something different.",
  ...
  },
  {
    ...
    example: "A shop plans for one till failing and keeps a spare. Nobody asked what happens if the power cuts out and both stop at once.",
  ...
  },
  {
    ...
    danger: "Can become an intellectualised form of procrastination — analysing your thinking instead of doing the thinking.",
    example: "A dripping tap has a known fix. A child struggling at school needs someone with expertise. A new business idea needs a small test first.",
...
const MTC_TOOLBOX = [
  { id: "ooda", name: "Notice, Understand, Decide, Act", formalName: "OODA Loop", summary: "Notice the facts, understand what they mean, choose a response, act, then check what changed.", when: "A fast-moving situation where you need to act and keep updating rather than wait for a perfect plan.", example: "Your train is cancelled. You read the board, work out which line still runs, take it, then check at the next stop whether you are still on track." },
  { id: "cynefin", name: "Choose the Right Response", formalName: "Cynefin Framework", summary: "A clear problem needs a known answer. A complicated one needs expertise. A changing one needs small tests. A crisis needs immediate stabilising action.", when: "Before choosing a thinking tool, when you first need to understand what kind of problem you face.", example: "A dripping tap has a known fix. A strange noise in the car needs a mechanic. A new market stall needs a trial Saturday before you commit." },
  { id: "mece", name: "No Gaps, No Overlap", formalName: "MECE", summary: "Divide a problem into groups that do not overlap and together cover everything important.", when: "Making a checklist or breaking down a problem without counting the same thing twice or leaving something out.", example: "Splitting the household budget into rent, food, travel and everything else, so no cost is counted twice and none is quietly left out." },
  { id: "five-whys", name: "Five Whys", summary: "Ask 'why' repeatedly on a problem until you reach an actionable root cause.", when: "Root-causing a recurring failure instead of patching its symptom.", example: "The bin keeps overflowing. Because nobody empties it. Because the rota is unclear. Because it was never agreed when people moved in." },
  { id: "map-territory", name: "The Plan Is Not Reality", formalName: "Map Is Not the Territory", summary: "A report, plan, label or memory is only a simplified picture. Check it against what is actually happening.", when: "A decision depends on a plan, dashboard, description or assumption that may be old or incomplete.", example: "The delivery app says the parcel arrived. You look on the doorstep and it is not there, so the record is wrong rather than the doorstep." },
  { id: "premortem", name: "Premortem", summary: "Imagine the project has already failed a year from now, then write down why.", when: "Before committing to a plan, to surface risks that optimism during planning suppresses.", example: "Before the party, you imagine it went badly and write down why: not enough seats, nobody told the neighbours, and no plan if it rains." },
  { id: "postmortem", name: "Postmortem", summary: "After the fact, reconstruct what actually happened and why, without blame.", when: "After any outcome (success or failure) worth learning from.", example: "After a holiday that went wrong, you work out that the flight was booked before the visa was checked, rather than deciding whose fault it was." },
  { id: "decision-matrix", name: "Decision Matrix", summary: "Score each option against weighted criteria in a grid.", when: "Comparing 3+ options on multiple, sometimes conflicting, criteria.", example: "Choosing between three flats by scoring each on rent, journey time, condition and noise, instead of arguing about which one felt nicest." },
  { id: "expected-value", name: "Expected Value", summary: "Multiply each outcome by its probability and sum, to compare bets on the same scale.", when: "Repeatable decisions where no single outcome is catastrophic.", example: "A £2 raffle ticket with a one-in-five-hundred chance of a £200 prize is worth about 40p, so the ticket is a donation rather than an investment." },
  { id: "pareto", name: "Pareto Principle (80/20)", summary: "Roughly 80% of effects often come from 20% of causes.", when: "Prioritising effort when time is short and a few causes account for most of the effect.", example: "Most of the mess in the kitchen comes from two things: the worktop by the kettle and the pile by the door. Fixing those two changes the room." },
  { id: "occams-razor", name: "Occam's Razor", summary: "Prefer the explanation with the fewest necessary assumptions.", when: "Choosing between competing explanations that fit the evidence equally well.", example: "Your friend has not replied. They are probably busy, rather than having read it, decided to ignore you and told other people about it." },
  { id: "hanlons-razor", name: "Hanlon's Razor", summary: "Don't attribute to malice what is adequately explained by incompetence or error.", when: "Interpreting someone's mistake before assuming bad intent.", example: "The invoice came with the wrong name on it. Someone was rushing far more likely than someone trying to cause you a problem." },
  { id: "inversion", name: "Inversion", summary: "Instead of asking how to succeed, ask how you'd guarantee failure — then avoid that.", when: "Stuck problems where the forward path is unclear but the failure path is obvious.", example: "Instead of asking how to have a good first week in a job, ask what would guarantee a bad one, then avoid exactly those things." },
  { id: "feynman", name: "Feynman Technique", summary: "Explain a concept in plain language as if teaching a child; gaps reveal what you don't actually understand.", when: "Checking whether you truly understand something or just recognise it.", example: "You try to explain your phone contract to a friend and get stuck at the second year, which is the part you never actually understood." },
  { id: "margin-of-safety", name: "Leave a Safety Buffer", formalName: "Margin of Safety", summary: "Keep extra time, money or capacity because estimates can be wrong and surprises happen.", when: "A delay, bill or failure would cause serious trouble if your plan had no spare room.", example: "Leaving at seven for an eight o'clock appointment that takes forty minutes, so one hold-up does not turn into being late." },
  { id: "eisenhower", name: "Eisenhower Matrix", summary: "Sort tasks by urgent/important into four quadrants to decide what to do, schedule, delegate, or drop.", when: "Prioritising a to-do list that is being driven by what is loudest, not by what matters.", example: "The boiler leaking gets done now. Booking the dentist gets a date. The parcel return can go to whoever is passing. The unread newsletter goes." },
  { id: "constraints", name: "Theory of Constraints", summary: "A system's throughput is limited by its single tightest bottleneck; optimising elsewhere doesn't help.", when: "Improving a process/system with one clear bottleneck.", example: "The kitchen serves food slowly because one oven is broken. Hiring another waiter changes nothing until the oven is fixed." },
  { id: "feedback-loops", name: "Feedback Loops", summary: "Reinforcing loops amplify change; balancing loops resist it — most system behaviour comes from these.", when: "Understanding why a system self-stabilises or spirals despite one-off interventions.", example: "The later you go to bed, the more tired you are, the more you scroll to feel awake, and the later you go to bed again." },
  { id: "leverage-points", name: "Leverage Points", summary: "Some places to intervene in a system produce far more change than others for the same effort.", when: "Choosing where to intervene in a complex system instead of just where the symptom is visible.", example: "Moving the shoes and keys to the front door saves more time each morning than getting up ten minutes earlier does." },
  { id: "black-swan", name: "Black Swan Thinking", summary: "Rare, high-impact, hard-to-predict events dominate long-run outcomes more than routine ones.", when: "Assessing tail risk that historical averages systematically understate.", example: "Years of routine journeys tell you little about the one flood that closes the road and decides whether the delivery business survives." },
  { id: "circle-of-competence", name: "Circle of Competence", summary: "Know the boundary of what you actually understand well, and stay inside it (or flag when you don't).", when: "Deciding whether your own judgement can be trusted on this particular subject.", example: "You can change a tyre and you cannot rewire a socket. Knowing which is which is worth more than being confident about both." },
  { id: "opportunity-cost", name: "Opportunity Cost", summary: "The value of the next-best alternative you give up by choosing this one.", when: "Any resource-allocation decision — the real cost is what else you could have done.", example: "Taking the overtime shift costs you the evening class you had already paid for, which is the real price of the extra money." },
  { id: "regression-mean", name: "Regression to the Mean", summary: "Extreme results tend to be followed by more average ones, without any real underlying change.", when: "Judging whether an intervention 'worked' after an extreme result (e.g. worst performer improving).", example: "A child has an unusually bad week at school, then a normal one. The tutor started on the Monday and takes the credit." },
  { id: "base-rates", name: "Base Rates", summary: "The background frequency of something in the population, which should anchor your estimate before adjusting for specifics.", when: "Estimating a probability — always start from the base rate, then adjust.", example: "Before deciding a cough is something rare, start from how many coughs in a normal winter turn out to be ordinary ones." },
  { id: "survivorship-bias", name: "Survivorship Bias", summary: "Studying only the 'survivors' of a selection process gives a distorted picture, because failures are invisible.", when: "Analysing what 'successful' people/companies/strategies have in common.", example: "Every shop still trading on the high street looks like proof the location works. The ones that closed are not there to ask." },
  { id: "signal-noise", name: "Signal vs Noise", summary: "Distinguishing a real, persistent pattern from random fluctuation.", when: "Reacting to a change in a metric — ask if it's outside normal variance first.", example: "Sales were down on Tuesday and up on Wednesday. Neither means anything until you look at several weeks side by side." },
  { id: "incentives", name: "Follow the Reward", formalName: "Incentives", summary: "People often do what a system rewards, even when that behaviour harms the real goal.", when: "A target, bonus, punishment or social reward may explain behaviour better than the stated intention.", example: "Staff are paid a bonus for closing tickets quickly, so tickets get closed quickly and reopened the next day by the same customer." },
  { id: "anchoring", name: "The First Number Sticks", formalName: "Anchoring", summary: "The first number or suggestion you hear pulls your thinking towards it, even when it has nothing to do with the question.", when: "Judging a price, offer, estimate or negotiation after someone else has named a number first.", example: "The first flat you view is £1,400. Every flat after that feels cheap or expensive compared with it, rather than with what you can afford." },
  { id: "loss-aversion", name: "Losing Feels Worse", formalName: "Loss Aversion", summary: "The pain of losing something often influences us more than an equal gain would.", when: "You are keeping a poor choice mainly because changing feels like accepting a loss.", example: "You keep a coat you never wear because getting rid of it feels like a loss, though you would not buy it again today." },
  { id: "sampling", name: "A Few Examples Prove Little", formalName: "Sampling", summary: "A conclusion is only as reliable as the number and variety of examples behind it.", when: "Reviews, complaints, surveys or personal stories are being used to make a broad claim.", example: "Two friends loved the restaurant. That is two evenings out of the hundreds it serves, which is why the booking still feels like a gamble." },
  { id: "activation-energy", name: "Make Starting Easier", formalName: "Friction and Activation Energy", summary: "The first step often needs the most effort. Reduce small obstacles so useful action is easier to begin.", when: "You repeatedly intend to start a habit or task but delay at the same first step.", example: "Putting the gym bag by the door the night before does more for going in the morning than deciding to be more disciplined." },
  { id: "diminishing-returns", name: "More Effort, Less Improvement", formalName: "Diminishing Returns", summary: "After the most useful work is done, each extra unit of effort may produce a smaller improvement.", when: "You need to decide whether to keep polishing one task or move effort somewhere more useful.", example: "The first twenty minutes of tidying transforms the room. The next twenty move three things and rearrange a shelf." },
  { id: "social-proof", name: "Everyone Else Is Doing It", formalName: "Social Proof", summary: "Seeing other people choose something can feel like evidence that it is safe or correct, even when the crowd may be mistaken or invented.", when: "Popularity, urgency or claims about other buyers are pushing you to decide quickly.", example: "A stall with a queue looks better than the empty one beside it, even when the queue is only there because it is slower." },
  { id: "attribution-error", name: "Person or Situation?", formalName: "Fundamental Attribution Error", summary: "We often blame someone's character before checking whether their situation explains the behaviour.", when: "A person's mistake or delay tempts you to label them before asking what happened.", example: "A colleague snaps at you and you decide they are rude. You later find out their train was cancelled and their childcare fell through." },
  { id: "action-bias", name: "Acting Is Not Always Helping", formalName: "Action Bias", summary: "Pressure can make any visible action feel better than waiting, checking or doing nothing.", when: "You feel forced to intervene before you know whether the action will improve the situation.", example: "The heating stops working and you start pressing buttons. Reading the display first would have shown it was simply set to off." },
  { id: "working-backward", name: "Work Backwards", formalName: "Working Backward", summary: "Start with the required result and time, then identify the steps that must happen before it.", when: "Planning a journey, deadline, event or task with a fixed finishing point.", example: "The train is at 9:10, the station is twenty minutes away, and you need ten to get ready. So you are getting up at 8:20, not 8:45." },
  { id: "scale", name: "Bigger Changes the Process", formalName: "Scale", summary: "A method that works for a few people may fail when the amount becomes much larger.", when: "A task, service or event is growing and simply multiplying the old plan may overload it.", example: "Cooking for four from that recipe is easy. Cooking the same thing for forty needs different pans, a different oven and a different order of work." },
  { id: "diversification", name: "Do Not Depend on One Thing", formalName: "Diversification", summary: "Spread important dependence across several sources so one failure cannot remove everything.", when: "Most of your income, supply, information or support comes from one place.", example: "If one person knows the door code, the accounts login and where the spare key is, then one holiday can stop everything." },
];
...
MTC_EXERCISES.push(
  ...
  { id: "warmup-5", type: "warmup", difficulty: 1, xpBase: 10, frameworks: ["abductive-reasoning", "cognitive-bias-detection"],
    ...
    expertNote: "Hanlon's Razor generalises here: before attributing to intent, exhaust the explanations that require nothing unusual about the person at all." },

  { id: "warmup-7", type: "warmup", difficulty: 1, xpBase: 10, frameworks: ["second-order-thinking", "strategic-thinking"],
    title: "Free Shipping",
    prompt: "An online store announces 'free shipping on all orders, forever.' Nothing is free. List two places the cost likely reappears, and one second-order effect on customer behaviour.",
    hints: ["If shipping revenue disappears but shipping costs don't, where must the money come from?", "Common absorptions: higher base prices, minimum-order thresholds later, squeezed supplier margins, or reduced service elsewhere.", "Second-order: free shipping changes ordering behaviour — more frequent small orders, higher return rates ('I'll just send it back'), which raises the very cost being hidden."],
    ...
    modelAnswer: "The cost typically reappears in higher base prices and in pressure on margins elsewhere (suppliers, support, packaging). Second-order: customers reorganize their behaviour around it — smaller, more frequent orders and casual returns — which inflates total shipping volume, so the policy partly manufactures the cost it absorbs.",
    expertNote: "Pricing moves are never just accounting — they're behavioural interventions, and the behaviour change is usually the bigger effect." },
...
MTC_EXERCISES.push(
  ...
  { id: "bias-7", type: "bias", difficulty: 2, xpBase: 20, frameworks: ["cognitive-bias-detection"],
    ...
    hints: ["One vivid positive trait is bleeding into unrelated ratings — the manager is rating a glow, not the dimensions.", "The halo effect: global impressions contaminate specific judgments, in both directions (one visible failure works the same way negatively).", "The resistant process rates each dimension independently, evidence-first: write the specific observed behaviours per dimension BEFORE assigning any score, and rate one dimension across all employees at a time rather than one employee across all dimensions."],
  ...
  { id: "observation-8", type: "observation", difficulty: 2, xpBase: 20, frameworks: ["analytical-thinking", "systems-thinking"],
    ...
    prompt: "A support team reports 'average first-response time improved 40% this quarter' right after a new bonus was tied to that metric. Before celebrating, what should you inspect — in the data and in the behaviour the bonus created?",
