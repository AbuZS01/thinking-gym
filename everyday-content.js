...
const MTC_EVERYDAY_REPLACEMENTS = [
  {
    ...
    steps: [
      ["What should you do first?", "Listen, check the order and speak calmly", "This finds the real problem without adding more anger.", ["Apologise straight away and start remaking the drink", "Explain that the shop is busy and ask them to wait", "Call the manager over before anything has been said"]],
      ["The drink was made incorrectly. What is the clearest response?", "Acknowledge the mistake and offer the shop's normal replacement", "A clear apology and practical fix deal with the real complaint.", ["Remake the drink without mentioning the error at all", "Add a free extra item on top of the replacement drink", "Explain how the order slip was taken and where it went wrong"]],
      ["The customer threatens a member of staff. What now?", "Step back and involve the manager or security under the shop rule", "Safety comes before finishing the complaint yourself.", ["Keep serving calmly and hope the customer settles down", "Offer a full refund at once so the person leaves sooner", "Ask them to step outside so the queue is not disturbed"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["What is the important new clue?", "The machine is making a sound it did not make before", "A change can warn of a developing fault even when the machine still runs.", ["The boxes coming off the line still look completely normal", "Nobody working nearby has mentioned or reported anything", "The shift ends soon and the machine is due to be switched off"]],
      ["What should you do if the stop button is safe to reach?", "Use it, keep people back and report the problem", "This follows every part of the rule without touching the machine.", ["Let it run to the end of the batch, then stop it", "Stop it and look inside to find what has come loose", "Note the time the sound started and carry on watching"]],
      ["Why should a trained person check it?", "The cause is unknown and opening it needs the right training", "Reporting uncertainty is safer than attempting a repair you are not trained to make.", ["The grinding sound is enough to show the motor has failed", "Anyone who heard the change should describe it in writing first", "Stopping the machine has already dealt with the whole problem"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["What should you avoid assuming?", "That the student has simply stopped trying", "Missing work can have many causes, and the facts given do not show which one applies.", ["That the missing work has been noticed by the student", "That a quiet moment to talk can be found this week", "That the school has guidance covering this situation"]],
      ["What is a useful private question?", "I noticed some work is missing. Is anything making it hard to complete?", "It names the fact and gives the student room to explain.", ["Is there a reason you have decided not to hand this work in?", "Shall we agree a date by which everything will be caught up?", "Would it help if I explained the task again from the start?"]],
      ["The student mentions feeling unsafe at home. What now?", "Follow the school's safeguarding procedure", "A possible safety concern must go to the people trained and responsible for acting.", ["Ask enough questions to work out how serious it really is", "Agree to keep what was said between the two of you", "Wait to see whether it is mentioned again next week"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["What does the ball suggest?", "A child may follow it into the road", "The ball and the visible shoes together create a clear developing hazard.", ["Someone has dropped rubbish from a parked car", "A game is being played safely on the pavement or sidewalk", "One of the parked cars is about to pull out"]],
      ["What is the safest immediate response?", "Slow down and be ready to stop", "Reducing speed creates more time and stopping distance if the child appears.", ["Sound the horn to warn whoever is there", "Steer wide around the parked cars", "Hold the same speed and watch the gap closely"]],
      ["Why is waiting for the child to appear weaker?", "It leaves less time and distance to stop", "Hazard awareness means acting on a strong warning, not waiting for the danger to be fully visible.", ["It assumes the child will look before stepping out", "It depends on the road behind staying clear", "It only works if the ball rolls back first"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["The alarm sounds. What should you do first?", "Stop work and use the marked route", "That is the first action on the rule provided in the scenario.", ["Finish the task you are part-way through", "Check with a colleague whether it is a drill", "Wait for a second alarm to confirm it"]],
      ["Where should you go?", "The stated meeting point", "The meeting point lets staff account for people and give further instructions.", ["The nearest exit door", "Wherever the crowd is heading", "The car park by the gate"]],
      ["You remember your phone inside. What now?", "Stay at the meeting point and tell the person in charge", "The rule clearly says not to return for belongings.", ["Go back for it now, before the area is closed off", "Ask a member of staff to go back and collect it for you", "Report it missing once the head count has been finished"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["What should come first?", "Move to a quieter safe place and speak calmly", "Reducing noise and attention can help the child regain control.", ["Explain carefully why this behaviour is not acceptable", "Offer a treat straight away so the noise stops", "Wait quietly nearby until the crying has finished"]],
      ["What is a clear short message?", "You are safe. We are going somewhere quieter now", "It gives reassurance and one simple next step.", ["Tell me exactly what has upset you so much", "If you stop now, you can choose something later", "Everyone here is looking at us, so please be quiet"]],
      ["When should you discuss what happened?", "After both of you are calmer", "Learning and problem-solving work better when the immediate overload has passed.", ["While it is happening, so the link is clear", "On the way home, before the details are forgotten", "Once someone else has heard your side of it"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["What is the main warning sign?", "The label and paperwork describe different contents", "A mismatch means the package cannot be handled with confidence.", ["The box arrived later in the shift than expected", "The label has been printed in unusually small text", "The outer packaging is a different colour from the rest"]],
      ["What should you do?", "Set it aside safely and tell the shift lead", "This follows the stated rule and prevents the box moving further by mistake.", ["Open it carefully to see which record is correct", "Correct the paperwork to match what the label says", "Send it on and note the mismatch on the delivery record"]],
      ["Why not assume it contains cloths?", "The damaged box and conflicting record leave the contents uncertain", "One label is not enough when another trusted record disagrees.", ["Labels are printed last and are the more reliable record", "The paperwork was written by hand and could be a simple slip", "Most boxes on this route do carry cleaning cloths"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["What makes the request a concern?", "It moves you alone into a dark, isolated place", "Isolation and poor visibility increase risk and reduce access to help.", ["The address given does not match the building number", "The message arrived after the delivery had started", "The parcel is smaller than the description suggested"]],
      ["What is a reasonable alternative?", "Offer a lit public meeting point and contact support if needed", "This uses the company guidance and still offers a way to complete the delivery.", ["Go in quickly so the delivery is not marked as failed", "Leave the parcel at the entrance and photograph it", "Ask the customer to explain why the alley is necessary"]],
      ["The customer refuses every safe option. What now?", "Mark it unsafe and follow support instructions", "You do not need to prove bad intent before using the safety process.", ["Complete it anyway, since the customer has already paid", "Agree a different time when the alley will be lit", "Ask a nearby resident to hand the parcel over for you"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["What should you check first?", "The item and the displayed refund policy", "The condition of the item and written policy determine the normal options.", ["How long ago the customer says it was bought", "Whether the same item is still sold in the shop", "How many people are now waiting in the queue"]],
      ["What can you offer under the normal policy?", "An exchange or store credit after the check", "Those are the options stated in the scenario.", ["A cash refund, since the item was clearly faulty", "A repair arranged through the shop at no cost", "A discount on the next purchase instead of a refund"]],
      ["The customer demands an exception. What next?", "Ask the manager to decide", "The policy clearly gives exception decisions to the manager.", ["Agree the exception and record why it was made", "Explain that no exception has ever been given here", "Offer store credit worth more than the item's price"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["What is the key fact?", "The confusion and unsteadiness are sudden changes", "A clear change from the person's normal state is the important warning.", ["The person has seemed quieter over recent weeks", "The room is warmer than it usually is at this hour", "Nobody else on the ward has reported feeling unwell"]],
      ["What does the care plan tell you to do?", "Stay with them and report at once to the nurse in charge", "Both actions are stated directly in the plan.", ["Help them to a chair and check again in an hour", "Write it in the daily notes for the next shift", "Ask them to describe how they are feeling first"]],
      ["Why should you avoid guessing the cause?", "Different causes need different trained assessment", "Reporting observed facts gets the person the right help without an unsupported diagnosis.", ["A guess recorded in the notes could be read as fact", "Naming a cause would worry the person unnecessarily", "The cause will usually become obvious within a day"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["What is the relevant problem?", "The ladder rocks because a foot is missing", "The equipment is unstable and therefore damaged.", ["The cable being fixed runs across the ceiling", "The job is expected to take only a few minutes", "A coworker is working within calling distance"]],
      ["What should you do with the ladder?", "Label it, remove it from use and report it", "That is the full site rule given in the scenario.", ["Ask a coworker to hold it steady while you climb", "Pack something firm under the missing foot", "Leave a note on it so the next worker knows"]],
      ["Why does a short job not change the answer?", "A fall can happen before the job is finished", "Short exposure can still lead to serious harm when equipment is unstable.", ["Short jobs are usually rushed and done less carefully", "The rule applies to every ladder on site equally", "A quick job still needs to be recorded afterwards"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["Which information should guide the decision?", "The warning printed on the bottle", "It directly covers the suggested action and comes from the product instructions.", ["How long the coworker has done this job", "How well the mixture worked for them before", "How much of each product is left"]],
      ["What should you do?", "Use the product as directed and do not mix it", "This follows the clear safety warning.", ["Try the mixture in a well-ventilated room", "Use a very small amount of the second product", "Apply them one after the other instead"]],
      ["The product is not working well. What is a safe next step?", "Stop and check the approved method or ask the supervisor", "A safer method can be found without breaking the warning.", ["Leave the product on the surface for much longer", "Use a stronger concentration of the same cleaner", "Ask the coworker to finish the job their way"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["Where should you stay with the child?", "In the public camera-covered area", "The stated procedure keeps the interaction visible and supported.", ["In the quiet staff room, away from the crowds", "Near the entrance where a parent would come in", "Walking the floor together to look for the parent"]],
      ["Who should you contact?", "The control room and a second staff member", "Both are required by the procedure in the scenario.", ["The control room, then wait for their instructions", "The nearest shop manager and the centre's reception", "A second staff member before anyone else is told"]],
      ["What should an announcement avoid?", "The child's name", "The procedure specifically protects that identifying detail.", ["The place where the parent should come to meet", "The request for the parent to speak to a member of staff", "The name of the centre making the announcement"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["What should you do with the bus first?", "Stop safely and secure it", "A moving bus adds danger to the unwell passenger and everyone else.", ["Continue to the next stop, which is close by", "Pull over only once the passenger looks worse", "Keep moving so help can meet you further along"]],
      ["What information should the emergency call include?", "What happened and the bus's location", "The call handler needs the situation and location to send the right help.", ["The passenger's name and where they boarded", "How far behind schedule the service now is", "What you think is most likely to be wrong"]],
      ["Who should guide any first-aid action?", "The emergency call handler and your training", "The stated rule connects action to trained guidance rather than guesswork.", ["A passenger who says they have medical experience", "The depot supervisor reached on the radio", "Whatever the passenger asks you to do for them"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["Does being a close friend prove permission?", "No, the desk cannot verify the claim from those words", "The relationship claim is not independent permission from the guest.", ["Yes, provided the visitor knows the guest's full name", "Yes, if the visitor can describe the guest accurately", "Only once the visitor has shown some identification"]],
      ["What should you say?", "I cannot confirm guest details, but I can take a message", "This follows the policy and offers a safe alternative.", ["Let me call the room and see whether they answer", "I can tell you whether they are staying, but not the room", "If you wait in the lobby I will try to find them"]],
      ["Why can you not even confirm the stay?", "That information could help someone locate or target the guest", "Privacy includes whether the person is present, not only the room number.", ["The booking system may not be fully up to date", "Guests are free to check out without telling the desk", "Confirming it would mean checking the record in public"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["Which change directly removes repeated walking?", "Place the tape within safe reach at each packing position", "It removes the repeated trip while respecting the stated movable item.", ["Keep a larger stock of tape at the central store", "Have one worker fetch tape for the whole table", "Move the packing table closer to the tape store"]],
      ["What should be checked before keeping the new layout?", "Whether movement falls and the path and tool rules still pass", "A small trial should measure the benefit and check that safety was preserved.", ["Whether the workers say the new layout feels better", "Whether more boxes were packed during the trial hour", "Whether the tape is being used up more quickly now"]],
      ["Why test a small change first?", "It reveals problems before every station is changed", "A reversible trial creates evidence at low cost.", ["A trial gives workers time to get used to the change", "One station is easier to return to its old layout", "It shows the supervisor that the idea is being taken seriously"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["Which label is clearer?", "Manager who approved this request", "It uses the meaning people need instead of an unexplained specialist phrase.", ["Name and role of the authorising stakeholder", "Approver details, as recorded in the system", "Person responsible for the approval decision"]],
      ["Where should short help text appear?", "Beside the question where the person needs it", "Help at the point of difficulty reduces searching and guessing.", ["In a short introduction at the top of the form", "In a linked guide people can open when stuck", "In a summary shown before the form is sent"]],
      ["How can you check the redesign?", "Ask a few new starters to complete it and note where they pause", "Watching real use reveals confusion that the writer may no longer notice.", ["Ask the team who wrote it to read it once more", "Count how many people finish it without asking for help", "Compare its length with the version it replaced"]],
    ],
  ...
  },
  {
    ...
    steps: [
      ["Which idea addresses both reported problems?", "Use wider soft handles and a flat reinforced base", "Wider handles spread pressure, while a flat base helps the bag stay upright.", ["Use a thicker material for the whole of the bag", "Add a second pair of handles lower down the sides", "Make the bag deeper so the shopping sits lower"]],
      ["What should be tested with customers?", "Comfort, stability, capacity and whether it still folds flat", "These checks cover the user's problems and every stated limit.", ["Whether the wider handles feel comfortable to hold", "Whether shoppers notice the design has been changed", "How much the new bag would cost to produce in bulk"]],
      ["Why involve people who have the problem?", "They can reveal difficulties the designer may not notice", "Designing with users gives evidence about real use rather than assumptions.", ["They are the people who will buy the finished bag", "Their views show which design looks most modern", "Involving them makes the change easier to introduce"]],
    ],
...
const MTC_MENTAL_MODEL_ADDITIONS = [
  {
    ...
    payload: {
      ...
      steps: [
        ...
        { ask: "What else must the plan protect?", options: ["£60 travel and the possible £80 replacement", "The £60 travel, since that cost is already certain this month", "Whatever is left after some optional shopping has been done", "Nothing further, because every regular bill has now been paid"], answer: 0, because: "Travel is known and the replacement is a stated risk. Together they use the remaining £140." },
        { ask: "Which choice keeps the buffer?", options: ["Set aside both amounts and delay optional spending", "Set aside the travel money and deal with the item if it breaks", "Spend part of it now and replace the buffer out of next month", "Keep the whole £140 back and travel only when it is unavoidable"], answer: 0, because: "This leaves the money available for both the known journey costs and the possible replacement." },
        { ask: "Why keep the £80 before the item breaks?", options: ["A buffer protects the plan when a realistic surprise happens", "The item is old enough that it is almost certain to fail soon", "Money set aside cannot be spent by mistake on something else", "£80 is roughly what most household replacements end up costing"], answer: 0, because: "A safety buffer handles uncertainty. It does not claim to predict the exact event." },
    ]},
  ...
  },
  {
    ...
    payload: {
      ...
      steps: [
        { ask: "What makes starting harder here?", options: ["Several setup steps and an easy distraction", "The course being longer than the time available each evening", "Not having decided which lesson should be studied next", "The evening being the point in the day when energy is lowest"], answer: 0, because: "Finding equipment and links adds effort, while the nearby phone offers an easier action." },
        { ask: "Which preparation removes friction?", options: ["Leave the laptop ready, save the lesson link and move the phone away", "Set a reminder on the phone for the time study should begin", "Write down which lesson comes next before going to bed", "Agree with someone else that you will both study at the same hour"], answer: 0, because: "The useful action becomes the easiest visible next step." },
        { ask: "What is a helpful first target?", options: ["Open the lesson and study for five minutes", "Finish one whole lesson before stopping for the evening", "Study for as long as concentration lasts on the night", "Set a target of three evenings across the coming week"], answer: 0, because: "A small start gets the task moving without requiring a large burst of effort." },
        { ask: "How should the change be judged?", options: ["Check whether starting became more consistent over several days", "Check how much of the course has been completed by the weekend", "Ask whether the evenings felt easier than they did before", "See whether the phone was picked up less often during study"], answer: 0, because: "Repeated starts show whether the changed setup is helping." },
    ]},
  ...
  },
  {
    ...
    payload: {
      ...
      steps: [
        { ask: "What would another hour change in the report?", options: ["Only small spacing and wording details", "Whether the reader understands the main finding at first reading", "How confident the manager feels about presenting the figures", "Whether any of the figures need to be checked once more"], answer: 0, because: "The scenario says the report is already correct, clear and ready." },
        { ask: "Where can the hour add more value?", options: ["Reply to the customer problem that still needs action", "Add a short summary at the front of the finished report", "Ask a colleague to read the report before it is sent out", "Prepare the notes you will need for tomorrow's meeting"], answer: 0, because: "The open customer problem can improve meaningfully, while further report polishing adds little." },
        { ask: "What is a sensible stopping rule?", options: ["Stop when the report is correct, clear and meets its purpose", "Stop once the time you set aside for the report has run out", "Stop when you can no longer find anything you want to change", "Stop after a second read produces no changes worth making"], answer: 0, because: "A clear quality threshold prevents endless work without removing necessary checks." },
        { ask: "When should you keep improving instead?", options: ["When another check could still prevent an important error", "When the report will be read by people outside the organisation", "When there is still time available before the deadline arrives", "When the subject is one you know less well than usual"], answer: 0, because: "Diminishing returns do not justify ignoring errors with serious consequences." },
    ]},
  ...
  },
  {
    ...
    payload: {
      ...
      steps: [
        { ask: "What should be checked first?", options: ["Whether the counting change created the apparent fall", "Whether the same fall appears in any of the other reports", "Whether a competitor launched something on the same day", "Whether the fall is large enough to matter over a whole month"], answer: 0, because: "A changed counting method can move the dashboard even when customer behaviour did not change." },
        { ask: "What comparison would help next?", options: ["Reliable order records across several similar days", "The same day last year, when the counting was unchanged", "The hourly pattern within the day the fall appeared", "What the dashboard showed in the week before the change"], answer: 0, because: "A wider, trusted comparison helps separate a real pattern from one unusual or miscounted day." },
        { ask: "The fall is real. What is the safer test?", options: ["Change one relevant thing for a limited group and compare the result", "Reverse the most recent change and see whether sales recover", "Ask customers directly why they bought less than usual", "Make all three changes but keep them small enough to undo"], answer: 0, because: "A small single change can show what helped without exposing the whole shop to an untested response." },
        { ask: "Why can immediate action be weaker?", options: ["Several rushed changes can cause harm and hide which one mattered", "Customers may notice sudden changes and lose confidence", "The team would have no time to prepare for the changes", "One day of data is rarely enough to justify any spending"], answer: 0, because: "Action is useful when it is informed. Pressure alone does not make a large response effective." },
    ]},
